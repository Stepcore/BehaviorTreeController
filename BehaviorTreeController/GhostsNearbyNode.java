		...
		if(closestGhosts.size()>1 && fleeing <= 0){
			...
			for(int i = 0; i<game.getJunctionIndices().length; i++){
				...
				if(game.getPacmanCurrentNodeIndex()==game.getJunctionIndices()[i]){
					//System.out.println("in junction " + i);
					...
					/*for(int j= 0; j<moves.size(); j++)
						System.out.println("moves possible: " + moves.get(j).toString());*/
					...
					for(int j = 0; j < moves.size(); j++){
						...
						for(int k = 0; k<illegalMoves.size(); k++){
							...
							if(moves.get(j)==illegalMoves.get(k)){
								//System.out.println("move: " + illegalMoves.get(k).toString() + " illegal");
								//moves.remove(j);
								noIllegals=false; //we say that this move is illegal and break this innerloop and carry on to the next possible move.
								break innerloop;
							}
						}
